...
exports.validateCategory = async (req) => {
  try {
    ...
    if(req.files?.image[0].size > 5 * 1024 * 1024){
      throw new customError(400, "this image size is not allowed image size maximum 5MB");
    }
    if(req.files.image?.length > 1){
      throw new customError(400, "this image size is not allowed image size maximum 1");
    }
